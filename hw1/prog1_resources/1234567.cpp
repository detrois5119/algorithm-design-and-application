...
vector<int> MIS(int i,int j){
	...
	if (j2!=-1&Data2D[abs(j2)][0]==i){
		...
		vector<int> ans4;
		ans2=sleep[i][j-1];
		ans3=sleep[i][Data2D[abs(j2)][0]-1];
		ans4=sleep[Data2D[abs(j2)][0]+1][j-1];
	    if(ans2.size()>=ans3.size()+ans4.size()+1){
			return ans2;	
		}
		else{
			ans3.insert(ans3.end(), ans4.begin(), ans4.end());
			ans3.push_back(Data2D[abs(j2)][0]);
		    ans3.push_back(j);
			return ans3;	
		}	    
	}
...
void MISS(int i,int j){
	...
	if(sleep2[i][j]==-1){
		if (j2!=-1&Data2D[abs(j2)][0]==i){
			...
			MISS(i,j-1);
			MISS(i,Data2D[abs(j2)][0]-1);
			MISS(Data2D[abs(j2)][0]+1,j-1);	
		}
		else{
			MISS(i,j-1);
		}
...
////		cout << sleep[0][last_num][i] << " " << sleep[0][last_num][i+1] << "\n";
...
//
//	for (int i=0;i<sleep[0][last_num].size();i=i+1)
